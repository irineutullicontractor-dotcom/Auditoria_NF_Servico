...
def extrair_nf(v):
    """Limpa a NF pegando apenas os números antes de qualquer barra ou ponto."""
    if pd.isna(v) or str(v).strip() == "" or str(v).lower() == "nan": return ""
    # Pega o que vem antes da barra, remove pontos e caracteres não numéricos
...
def estruturar_notas_produtos_interno(file):
    ...
    registros, cnpj_dest, colunas_id, processando = [], None, None, False
...
if st.button("🚀 Iniciar Auditoria"):
    if all([file_nf_prod, file_forn, file_painel, file_relacao, file_contrato]):
        ...
        # 2. Padronização e Chaves de Cruzamento
        ...
        # 3. Tratamento Painel (Lógica NF Lançada)
        ...
        def status_pedidos(r):
            ...
            if r['CNPJ emitente'] in cnpjs_com_pedido: return "⚠️ Para Verificação"
            return r['Status']
        ...
        # SELEÇÃO DE COLUNAS (Limpeza de poluição)
        ...
        st.download_button("📥 Baixar Relatório Consolidado", output.getvalue(), "AUDITORIA_PRODUTO_FINAL.xlsx")
